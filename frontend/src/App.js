...
function App() {
  ...
  const [gameActive, setGameActive] = useState(false); // Tracks if a game is currently active
  const [currentTurn, setCurrentTurn] = useState(false); // Tracks if it is the player's turn

  // Handles connecting to the wallet
  const handleConnectWallet = async () => {
    if (typeof window.ethereum !== 'undefined') { // Checks if MetaMask (or another Ethereum provider) is installed
      try {
        await window.ethereum.request({ method: 'eth_requestAccounts' }); // Request account access from MetaMask
        setWalletConnected(true); // Update state to indicate wallet connection
        document.getElementById('connectWallet').textContent = 'Connected'; // Update button text
        enableGameButtons(); // Enable game-related buttons
      } catch (error) {
        console.error('User denied account access'); // Log error if user denies connection
      }
    } else {
      alert('Please install MetaMask!'); // Prompt user to install MetaMask if not found
    }
  };
